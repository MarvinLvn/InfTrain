...
## CV_DATASET            the location of the zerospeech dataset used for evaluation (default:/gpfsscratch/rech/cfs/commun/cv21_ABX/raw_dataset)
...
for lang in ${LANG}; do
        ...
        PATH_ITEM_FILE="${DATA}/${CONTRAST}.item"
        ...
        if [ ! -f ${PATH_OUT}/ABX_scores.json ]; then
            ...
            srun python $ABX_PY from_checkpoint $CPC_CHECKPOINT_FILE $PATH_ITEM_FILE --speaker-level 0 $DATA --seq_norm --strict --abx_analysis --file_extension $FILE_EXT --out $PATH_OUT
        fi;
done

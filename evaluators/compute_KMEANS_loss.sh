...
##
## Usage: ./evaluate_KMEANS_loss.sh /path/to/duration/family_id
##
## Example:
##
...
LOSS_PY="${LOSS_PY:-/gpfsscratch/rech/cfs/uow84uh/InfTrain/utils/validate_KMEANS.py}"
...
if [ -d "${CHECKPOINT_LOCATION}/kmeans50" ]; then
  ...
  OUTPUT_LOCATION="${CHECKPOINT_LOCATION}/kmeans50/scores/CommonVoiceLoss"
else
  die "No KMEANS checkpoints found for family ${FAMILY_ID}"
fi

...
##
## 1) Extract quantized units (scripts/quantize_audio.py) on /gpfsscratch/rech/cfs/commun/marvin_eval/child_syntax/syntactic
...
ZEROSPEECH_DATASET="${ZEROSPEECH_DATASET:-/gpfsscratch/rech/cfs/commun/marvin_eval/child_syntax}"
...
do
  ...
  if [ "$ARGUMENTS" == "None" ] ; then
    python "${BASELINE_SCRIPTS}/compute_proba_${MODEL_TYPE}.py" "${quantized}" "${output}" "${LM_CHECKPOINT_FILE}" --pooling mean
  else
    python "${BASELINE_SCRIPTS}/compute_proba_${MODEL_TYPE}.py" "${quantized}" "${output}" "${LM_CHECKPOINT_FILE}" "${ARGUMENTS}" --pooling mean
  fi
done
...
zerospeech2021-evaluate --no-phonetic --no-lexical --no-semantic --njobs $NB_JOBS -o "$OUTPUT_LOCATION/scores/child_sblimp" $ZEROSPEECH_DATASET $FEATURES_LOCATION

# copy the score on $SCRATCH
mkdir -p $CHECKPOINT_LOCATION/$MODEL/scores/child_sblimp
cp -r $OUTPUT_LOCATION/scores/child_sblimp $CHECKPOINT_LOCATION/$MODEL/scores

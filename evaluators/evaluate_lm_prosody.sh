...
ZEROSPEECH_DATASET="${ZEROSPEECH_DATASET:-/gpfsscratch/rech/cfs/commun/prosodic_evaluation_wn003/prosodic_lex}"
...
zerospeech2021-evaluate --no-phonetic --no-lexical --no-semantic --njobs $NB_JOBS -o "$OUTPUT_LOCATION/scores/prosody_lex_wn003" $ZEROSPEECH_DATASET $FEATURES_LOCATION

# copy the score on $SCRATCH
mkdir -p $CHECKPOINT_LOCATION/$MODEL/scores/prosody_lex_wn003
cp -r $OUTPUT_LOCATION/scores/prosody_lex_wn003 $CHECKPOINT_LOCATION/$MODEL/scores

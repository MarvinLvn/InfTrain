...
BEST_VAL_SCRIPT=/private/home/marvinlvn/InfTrain/utils/best_val_epoch.py
if [ "$#" -ne 2 ]; then
...
if [ -d $CPC ]; then
  ...
  if [ ! -d ${MODEL_PATH}/${CPC}/ABX/${BEST_EPOCH} ]; then
    echo ${PATH_DB} >> experiments_txt/cpc_small_eval_experiments.txt
  fi;
fi;
